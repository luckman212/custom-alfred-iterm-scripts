...
on is_processing()
  tell application id "com.googlecode.iterm2" to tell the first window to tell current session to return is processing
end is_processing
...
on alfred_script(query)
  ...
  -- "with timeout" does not work with "repeat", so use a delay (0.25 * 25 means a timeout of 5 seconds)
